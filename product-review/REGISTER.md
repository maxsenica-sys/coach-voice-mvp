...
| WOW-001 | 2026-09-06 | CLOSED | Hear It — persist the Whisper segments the app already computes and discards; every coaching point becomes playable in the coach's real voice | PROTOTYPE | 37.5 |
...
| WOW-002 | 2026-09-06 | CLOSED | "The Line" — one stroke that is waveform, silhouette and logo, driven by a real coach's real 8 seconds, playing silent because autoplay is blocked | PROTOTYPE | 21.3 |
...
| DESIGN-010 | 2026-09-23 | IMPLEMENTED | "Stadium Night" — the whole app moved onto the ink ground: broadcast furniture, one enormous number per screen, one new hue (floodlight `#CBEF5E`) spent on three things only | BANK | not scored |
| DESIGN-011 | 2026-09-23 | APPROVED | "Floodlit" — attention directed by illumination rather than by cards or borders: one warm off-frame key on a green-black ground, one new hue (mercury `#7FC6D6`) reserved for the athlete's own voice | BANK | not scored |
| DESIGN-012 | 2026-09-23 | REJECTED | Nineteen further redesign directions, all rejected by Max after review — light grounds decisively out, metaphor-led directions out, and restyling a winner did not produce a second one | REJECT | not scored |
| DESIGN-013 | 2026-09-24 | IMPLEMENTED | Stadium Night drawn end to end — eighteen screens from one written spec, the chartreuse discipline holding across all of them, and nine decisions surfaced that are product calls rather than drawing problems | DECIDE | not scored |
| DESIGN-014 | 2026-09-25 | IMPLEMENTED | The floor under Stadium Night: 13px type floor (154 runs to 0), no sideways scrolling, summary drafted at stop and editable before it sends, no data removed to fit | BUILD | not scored |
| DESIGN-015 | 2026-09-25 | IMPLEMENTED | Stadium Night built into every page: the token layer flipped to ink, then each surface rebuilt to the DESIGN-013 spec (coach home, roster, invite, athlete home and profile, session page, recorder, messages, sign-in, cold start, both PDFs, share page, Focus Card) | BUILD | not scored |
...
### Prototypes are no longer a thing this project does — 2026-09-09

Max: *"get rid of the prototype, i dont want that or anything like that in the
future."* `/dev/hearit` (WOW-001) and `app/components/_banked/` (the banked
intro directions B and D) are deleted, along with `/dev` in the proxy matcher
and its coach-only route rule. The standing instruction is written into
`CLAUDE.md`.

WOW-001 and WOW-002 are marked **CLOSED**, not BACKLOG. The round-2 report said
of WOW-001: *"if nobody asks to hear it again, close it rather than backlogging
it."* Nobody did.

**What this changes about how ideas get proposed here.** An idea is now either
built into the product properly, or written down in `product-review/` and not
built. There is no third state where it half-exists in the tree. Any agent
recommending "build a one-day prototype at /dev/x" as its cheapest test should
expect that recommendation to be rejected on principle — the cheap test has to
be something other than a parallel page nobody maintains.
...
| ~~2026-09-09 DESIGN-008~~ | ~~Design~~ | **DELETED 2026-09-25** with DESIGN-015: on ink its three tones were two light colours and a rust, which no one text colour reads on, and the rebuilt screens stopped calling it. Was: **DONE 2026-09-09** — `stableTone(id)` in `lib/group-colors.ts` hashes the row id (FNV-1a), so an athlete keeps one colour everywhere and adding a teammate re-colours nobody. |
...
| ~~2026-09-23 DESIGN-010~~ | ~~Design~~ | **CORRECTED same day — not a live defect.** `--coach-color #B55C3E` is 3.43:1 on ink, but every current use of it is a *background* with white text on top (message bubbles, the athlete header, a 5px dot, three gradients). The app never renders rust text on ink today. It is a constraint on building Stadium Night, which does; `#E39A7A` (6.83:1) is the value that direction needs. Verified by grepping every call site. |
| ~~2026-09-23 DESIGN-010~~ | ~~Design~~ | **DONE 2026-09-23** — the real one, found the same way: `--energy-dark` on `--wellness-ok-tint` was **4.05:1** and failed 1.4.3 in two places, the "Working on it" chip and the "Modified" injury status. One wrong token; `--wellness-ok` is 4.55:1 on the same tint. `verify:palette` now imports `SESSION_RESPONSES` and `INJURY_STATUSES` and checks every declared colour/tint pair — the check that was missing. |
...
| 2026-09-23 DESIGN-011 | Design | Floodlit is banked unbuilt under the **same daylight gate**, and it is the more exposed of the two: its whole hierarchy is carried by a sodium pool, which is the first thing to vanish on a bright touchline. It also needs a designed empty state and overflow state before it is a system — it assumes exactly one subject per screen, and a coach with nine athletes needing him, or none, breaks that. |
| ~~2026-09-23 DESIGN-012~~ | ~~Product~~ | **CORRECTED 2026-09-25 — it always was a field.** `lib/summary-prompt.ts` asks for it by name as the `NEXT:` line, parses and caps it, and prompt-rig tests it. The claim below was inferred from `focus_points[0]` at the read site and never followed back. Kept struck through because the wrong claim is the useful part. ~~**The key takeaway is not a field.** Three independent directions bet the design on it and all three hit the same wall: it is `focus_points[0]`, whichever bullet the summariser emitted first. Any future direction that leans on it first needs it asked for explicitly in the prompt, pinned with its own `tools/prompt-rig.mjs` case, and coach-editable for a few minutes after recording. ~1 day, and a prerequisite rather than a detail. Survives the rejection of all three designs that surfaced it. |
| ~~2026-09-23 DESIGN-010~~ | ~~Design~~ | **CORRECTED 2026-09-24.** The banked palette listed `--cream-3` at 5.41:1, computed against flat ink. On the real composited ground (beam + 39px grid + grain) it measures **3.76–4.48:1**, and the two approved boards ship the failure — `TUE 23 SEP` at 3.74:1, `TRENDS →` at 3.07:1. The dominant contributor is the grain, not the beam: at `.22` even `--cream-2` fails at 4.37:1. Retire `.58` as a text colour, damp the grain to `.13`. Third time this class of bug has appeared; measure pixels, not hex. |
| 2026-09-24 DESIGN-013 | Design | The two approved Stadium Night boards have the **wrong weekdays** — `TUE 23 SEP` is a Wednesday in 2026 and `FRI 19 SEP` is a Saturday. Nothing type-checks a date written into a mockup, and it would have shipped. |
| 2026-09-24 DESIGN-013 | Safeguarding | The roster concept surfaces `28d QUIET` beside a child's name, sortable, in the alarm colour, on a phone held courtside where a parent or the athlete can read it over the coach's shoulder. `lib/attention.ts` is explicit that this ranking is coach-only and deliberately says "Inactive". The marker must vanish on a handed-over phone, and a real coach should be asked whether the column reads as a tool or an accusation. |
| 2026-09-24 DESIGN-013 | Correctness | Under `prefers-reduced-motion` a **stopped recording looks identical to a running one** — the frozen VU is the only proof of life. Needs a non-luminance second signal (a ticking seconds digit, free from the existing clock). A defect in the design, not a preference. |
| 2026-09-25 DESIGN-014 | Coverage | **The boot harness proves "nothing scrolls sideways" on three public routes only.** The dashboard, athlete home and athlete profile carry the widest content and need a session the harness cannot create. They were verified by per-file browser harnesses during the build, which are not in CI. Until the harness can sign in, a regression on those pages is invisible — and with horizontal overflow clipped, invisible means missing content, not a scrollbar. |
| 2026-09-25 DESIGN-014 | Correctness | A **muted** (not ended) audio track — an iOS call interruption — still ticks the recorder's new proof-of-life clock. Left out because `muted` flickers true on some browsers and a false "Recording stopped" is worse than the gap. Wants a >1.5s debounce. |
| 2026-09-25 DESIGN-014 | Design | Big Shoulders ships with no metric-override fallback (`next build`: "Failed to find font override values"), so every uppercase label will reflow on the late swap. Currently unused, so no cost yet; it becomes a CLS cost the day Stadium Night applies `--font-cast`. |
| 2026-09-25 DESIGN-014 | Product | **Volleyball is not in the sport picker's fourteen**, though every Stadium Night mockup is set in it. |
| ~~2026-09-25 DESIGN-015~~ | ~~**Safeguarding**~~ | **DECIDED by Max 2026-09-25 ("do 1 and 2") and built.** (1) An athlete's check-in notes are left out of anything a parent receives by default: the caretaker wellness email carries the note only when `audience === 'coach'` (`lib/notify.ts`, held by safeguard rule **SG9**), and the monthly report prints no note unless the coach ticks it in, one by one. (2) The check-in tells the athlete who reads what they write: "Only your coach sees this". |
| ~~2026-09-25 DESIGN-015~~ | ~~**Safeguarding**~~ | **FIXED 2026-09-25.** An athlete's **injury update never reached anyone**. The check-in asks it while an injury is open, the API stored it and returned it to the coach's athlete page, and no screen rendered it. The coach's wellness card now shows the newest one with its date. It is in no report and no parent email. |
| ~~2026-09-25 DESIGN-015~~ | ~~Correctness~~ | **FIXED 2026-09-25.** The monthly report **printed Soreness, Stress and average wellness upside down**. It still computed `6 - soreness` / `6 - stress` after the inversion was deleted on 2026-09-09 (the entry above), so the one document a parent receives ran backwards for two weeks. It now normalises through `WELLNESS_METRICS`. |
| ~~2026-09-25 DESIGN-015~~ | ~~Correctness~~ | **FIXED 2026-09-25.** A search on the Sessions tab replaced the list Home counts from, so "sessions this week", the spark and Latest silently became counts of the search matches. |
| 2026-09-25 DESIGN-015 | Product | **The attention headline is back on the coach's home** ("Three athletes haven't had a session in two weeks"), because the approved DESIGN-013 board has it. `lib/attention.ts` records Max having the quiet strip taken off home on 2026-09-10. Worded as a fact about sessions, links to the Inactive filter, coach-only. It is the judgement in this build most worth Max looking at. |
| 2026-09-25 DESIGN-015 | Design | DESIGN-014's Big Shoulders note is **now a live cost**: `--font-cast` is on every uppercase label in the app, and the face still ships with no metric-override fallback, so each one reflows when the font lands. |
| 2026-09-25 DESIGN-015 | Product | The athlete's message thread is headed "YOUR COACH" because the athlete page does not load the coach's name. Naming them needs a data change, not a design one. |
| 2026-09-25 DESIGN-015 | Safeguarding | The share-clip page is authenticated (owner coach or the athlete it was shared with), but its title is the clip's `file_name`, which a coach may have named after the child. |
| ~~2026-09-25 DESIGN-015~~ | ~~Correctness~~ | **FIXED 2026-09-25.** Date-only strings (`check_date`, `event_date`) went through `new Date('YYYY-MM-DD')`, which is UTC midnight, so west of UTC a check-in printed as the day before. And `yesterdayISODate` subtracted 86,400,000ms, which at 00:30 after a 23-hour day names the date two days back. Both fixed, and clock-rig P8b/P8c hold them; both went red against the old code. `fmtDate` and `fmtDateTime` had no callers and are deleted. |
| 2026-09-25 DESIGN-015 | Design | Open for Max: the video annotator's pen offers the stock nine colours; the Stadium Night spec proposes five drawn from the system. |
| ~~2026-09-26 audit~~ | ~~UX~~ | **DONE 2026-09-26.** The recorder's athlete chips sat in a 132px scroll box: three rows of a twenty-athlete roster. Now AthletePicker: search, squad filter, a full list that scrolls with the sheet, and it folds to one row once picked. Matching by word start, held by verify:roster R5. |
| ~~2026-09-26 audit~~ | ~~UX~~ | **DONE 2026-09-26.** The roster arrived newest-invited-first and nothing re-sorted it; Home showed only the 8 newest; full-name search matched nobody; the tab and the roster search were lost on every trip into a profile. All fixed: sorted at load, Home shows everyone (with a last initial), word-start search, `?tab=` in the URL, search kept for the session, profile back goes to the roster. |
| ~~2026-09-26 audit~~ | ~~**Safeguarding**~~ | **FIXED 2026-09-26.** Three ways a child's data landed on the wrong child: a queued squad recording that partly saved was re-POSTed to every member on each re-render (duplicate sessions); a draft or voice note in Messages followed the coach to the next athlete; after a re-record, one athlete's AI summary could be saved on another's session. |
| ~~2026-09-26 audit~~ | ~~Correctness~~ | **FIXED 2026-09-26.** Check-ins were stored under the server's UTC date; before about 10am in Australia they landed on yesterday. The phone now sends its own date, and the route accepts it only within a day of UTC today. |
| ~~2026-09-26 audit~~ | ~~UX~~ | **DONE 2026-09-26 (built the same day).** Was pencilled: Switch athlete from inside the profile: tap the name for a search-first sheet (UX-011, M). The calendar tab on a phone buries the chosen athlete's calendar ~1000px down under a 44px-per-athlete sidebar (M). Sessions tab stops at 50 with no Load older, though the API returns `hasMore` (S–M). Messages does not put unread threads first (S). Squad building is one select + one tap per athlete (S–M). Calendar shows an athlete's event to the coach as "My event" (S). |
| ~~2026-09-26 audit~~ | ~~UX~~ | **DONE 2026-09-26 (built the same day).** Was pencilled, athlete side: Readiness sits below a 690px body map, so the two-tap check-in is a scroll plus two taps (M; moving readiness above is S). The check-in shows both "Selected:" and "Marked:" with contradictory empty messages (S). A readiness-only day is shown as five wellness numbers the athlete never gave (S–M). Joining a coach from the portal hides its own confirmation and does not load sessions (S). The signup sport filter picks "Table Tennis" for "tennis" (S). Raw ISO dates shown to athletes (S). No confirm on deleting a private note or on the one-tap Out (S). |
| ~~2026-09-26 audit~~ | ~~Correctness~~ | **DONE 2026-09-26 (built the same day).** Was pencilled: A mismatched coach code at signup is dropped silently (S–M). If Supabase email confirmation is on, signup fails with "Unauthorized" and loses the profile (M; check the setting first). `/auth/callback` reads only legacy cookie names, so a password-reset link may not complete (S; needs one real reset email to confirm). Several loads show an empty state on failure instead of an error: athlete notes and sessions, the coach's notes tab, the day wheel, InjuryPanel's "available" (S each). Silent failures on add-to-squad and hard delete (S). Cancelling a voice message brings it back (S). A failed focus-point save loses the typed text (S). |
| ~~2026-09-26 build~~ | ~~**Security**~~ | **FIXED 2026-09-26.** `/api/complete-signup` accepted `{ role: 'coach' }` from any signed-in user at any time, so an athlete could make themselves a coach. It now refuses a completed profile and never turns an athlete into a coach. |
| ~~2026-09-26 build~~ | ~~Correctness~~ | **FIXED 2026-09-26.** The proxy's signed-in test matched the PKCE `…-auth-token-code-verifier` cookie, so a visitor who had just requested a reset was fast-pathed to /dashboard and bounced back. Boot check added; it goes red on the old pattern. |
| ~~2026-09-26 build~~ | ~~Product~~ | **DECIDED by Max 2026-09-26:** "Nothing sore" stays pre-selected. Two taps is right; an athlete who picks it is not asked to expand. |
| 2026-09-26 roadmap | Product | **APPROVED by Max 2026-09-26, batch 1 (building):** record first, pick the athlete after; several athletes in one recording with the summary split per athlete and reviewed before saving ("I'd rather talk and include the athletes in one message and then it can split"); pre-session brief **20 minutes** before, flagging who has not checked in, with a nudge; weekly digest and takeaway reminder **inside the app only**; coach insights (what you repeat, replies as a signal, coverage fairness); audit log of when an athlete opens their information; invite by QR code; offline check-in. |
| 2026-09-26 roadmap | Product | **APPROVED by Max 2026-09-26, batch 2 (after batch 1 ships):** push notifications; video (clip onto a takeaway, side-by-side, athlete uploads); assistant coaches (a permissions change across every coach query, so planned separately). |
| 2026-09-26 roadmap | Product | **REJECTED by Max 2026-09-26:** session templates; goals linked to sessions; streaks ("too childish"). Name detection in transcripts: Max unsure, not building. Do not re-propose without new evidence. |
| 2026-09-26 build | Ops | `/auth/callback?next=/signup?finish=1` should be on Supabase's Redirect URLs allow-list for the confirm-email path to finish in one step; without it the next password sign-in finishes the account. |
